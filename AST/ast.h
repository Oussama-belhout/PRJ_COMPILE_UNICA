...
typedef struct InstructEntry {
    struct AST *instruct;
    struct InstructEntry *next;
} InstructEntry;
static InstructEntry* curr_bloc = NULL ;
static InstructEntry* prev_bloc = NULL ;
...
AST *ast_new_block() {
    ...
    if (node) {
        node->tag = AST_BLOCK;
        node->data.AST_BLOCK.instructs = curr_bloc;
    }
    return node;
}
...
void add_bloc_instr(AST* instr) {
    ...
    instr_entry->instruct = instr;
    instr_entry->next = curr_bloc;
    curr_bloc = instr_entry; // Update curr_bloc to point to the new entry
}
    ...
    switch (node->tag) {
        ...
        case AST_BLOCK:
            ...
            for (int i = 0; i < node->data.AST_BLOCK.count; ++i) {
                ...
                char new_prefix[256];
                snprintf(new_prefix, sizeof(new_prefix), "%s%s  ", prefix, is_last ? " " : "|");
                ast_print_tree(node->data.AST_BLOCK.instructs->instruct, new_prefix, is_last_inst);
            }
            break;
